...
export default function TabLayout() {
    ...
    React.useEffect(() => console.log("stackRoute", stackRoute), [stackRoute])
        ...
        <Tab.Navigator
            ...
            screenOptions={({ route }) => ({
                ...
                header: ({ navigation, route, options }) => {
                    return !routeExclude.includes(stackRoute)  && <resetForm.Provider value={chunk}><HeaderLayout headerStyle={options.headerStyle} option={route.name} navigation={navigation} /></resetForm.Provider>
                ...
                tabBarStyle: {
                    height: 70,
                    display: tablessRoute.includes(route.name) || routeExclude.includes(stackRoute) ? 'none' : 'flex',
                },

            })}
        >
            <Tab.Screen name="Home" options={({navigation, route }) => navigation.isFocused() && getHeaderTitle(route)} component={HomeStackNavigation} />
            ...
            <Tab.Screen name="Message" options={({navigation, route}) => navigation.isFocused() && getHeaderTitle(route)} children={({ navigation }) => <UserGate navigation={navigation}><MessageNavigationStack navigation={navigation} /></UserGate>} />
